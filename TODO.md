...
- [ ] ボール保持者の「判断力」パラメータ（decision）導入。判断力が高い選手ほど状況変化への再判断が速く、選んだプレーをキャンセルできるようにする。`specification/選手思考の状態遷移を検討.md`のIntent構想（interruptible等）が土台になる想定（2026-08-22 検証事項として提起）
- [ ] オフサイド判定・反則・カード（ステップ2以降: 反則としてのターンオーバー処理、守備のオフサイドトラップ連携。`specification/features_offside.md` 参照）
- [ ] スローイン/コーナー/ゴールキックの厳密化、ゴール線判定の精密化。現状`handleOutOfBounds`（[match.ts](src/game/match.ts)）はキックの種類（パス/シュート/クリア）やピッチ外に出た場所（タッチライン/ゴールライン）を区別せず、一律「相手チームの中央最寄り選手に中央付近で持たせる」簡易再開のみ。クリア導入（2026-08-22）でボールがピッチ外に出る頻度が増えたため、厳密化に着手する段階に来ている
