...
- オフサイド判定 ステップ2（反則としてのターンオーバー処理、マイルストーンJ〜M）：**複数回試みたが撤回**。反則有効化時に平均得点がほぼ0まで崩壊する問題が未解決。マイルストーンOでバグ修正後に再検証したが、同じ構造的限界（反則率を下げると団子化で崩壊）が再現し、依然未解決（詳細は [`TODO_ARCHIVE.md`](TODO_ARCHIVE.md) マイルストーンM/O）。`config.ai.offside.enabled`/`enforcementEnabled` は `false` のまま維持
...
- [x] マイルストーンOで `scoreReceivingSpot` の単位不一致バグ（`offsideOvershootWeight` がメートル単位のまま無次元の `advanced` と加減算されていた）を修正し、`avoidanceEnabled` を再チューニングして実用化した。この修正後に `enforcementEnabled` も改めて `forwardReachFraction`/`offsideOvershootWeight` のグリッドサーチで再検証したが、反則率を50%台→20%台まで下げようとすると平均得点が2.1前後→0.2〜0.4まで急落する団子化が再現し、両立点は見つからなかった。**「バグのせいで見えていなかっただけ」ではなく、正規化後も同じ構造的限界が残っている**ことが確認できた（詳細: [`TODO_ARCHIVE.md`](TODO_ARCHIVE.md) マイルストーンO）
- [ ] 受け手ポジショニング側の改善はマイルストーンKで打ち切り。再挑戦するなら以下のいずれかが必要:
  - 攻撃側の選択肢強化（ドリブル・短いパス等の代替ルート）
  - 反則発生率を一桁台まで下げる別アプローチ（静的なオフセット計算ではなく、オンサイドの状態から走り込むタイミングを考慮した動的モデルへの作り直しが本命候補）
