...
- [ ] 受け手ポジショニング側の改善はここで打ち切り。再挑戦するなら攻撃側の選択肢強化（ドリブル・短いパス等の代替ルート）か反則発生率を一桁台まで下げる別アプローチが必要。`config.ai.offside.enabled` は `false` のまま維持
...
- [ ] 結論: 「受け手ポジショニングの質」「パス/ドリブルの選択」レベルの改善はすべて崩壊を止められなかった。`avoidanceEnabled`有効時に前進そのものが止まる問題は、これらより支配的な別の要因（ゾーン守備との相性、選手数の少なさ、あるいは未特定の別要因）にあると考えられる。次に調査するなら、まず「守備をゾーンからマンマーク寄りに変えると数的優位実験の傾向が明確になるか」を軽量な実験で確認するのが筋が良さそうだが、大掛かりな再設計になるため未着手。`config.ai.offside.avoidanceEnabled`/`enforcementEnabled` は `false` のまま維持
