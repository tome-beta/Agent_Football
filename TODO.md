...
- [ ] `positioning.minSpacing`（味方同士の反発が働き始める距離、既定6m）を上げて陣形をもっと広げる案は保留（2026-08-16）。balance-checkでは8mまでは崩壊なし・10m以上で得点減衰と偏りが出始めることを確認済みだが、`avoidanceEnabled=false`のままだと受け手が前に張り出す抑制が効かず「なぜそこにいるか」の説明がないまま散らばるだけに見える、というユーザー指摘があった。オフサイド回避が機能してから再検討する
