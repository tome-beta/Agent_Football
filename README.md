...
- **ステータス**: **マイルストーンA〜M 完了/決着済み**（第一ステップ MVP、Release v1.0 相当）
  ...
  - オフサイドの反則化（ステップ2、マイルストーンJ〜M・O）は複数回試みたが、有効化すると平均得点がほぼ0まで崩壊する問題が長く未解決だった。マイルストーンPで「反則率をゼロに近づける」前提を見直し、mentalが高い（積極的な）選手だけがオフサイド覚悟の縦パスを低頻度で試みる（時には成功、時には反則）設計に転換して解決。`config.ai.offside.enforcementEnabled` は既定 `true`（2026-08-16）。詳細・根本原因の調査記録は `TODO_ARCHIVE.md` マイルストーンM/O/P参照
  - 次の作業: 第二ステップ（`TODO.md` の「未完了タスク」参照。スタミナ・GK AI・オフサイドトラップなど）
