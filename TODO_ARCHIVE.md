...
`TODO.md` から切り出した、完了済み・撤回済みのタスクの詳細記録。現在地・未完了タスクは `TODO.md` を参照。
...
#### 受け手ポジションの「横一列固定陣形」問題を修正（2026-08-08、実試合を見てのユーザー指摘）

「ボールを持っている側の非保持選手が、ポジションを固定しているのかと思うくらい同じ形で動く」という指摘を受けて `computeTargetPosition`（味方保持時の分岐）を調査した。

- [x] **原因判明** — 受け手ポジションの `target.x` は `home.x` と50/50でブレンドしていたが、`target.y` は `openSpot.y`（=ボール位置基準の受け手候補地点のy座標、全員で共通の値）をそのまま採用していた。そのため非保持の2選手は常に「ボールと同じ前進距離・同じ高さ」に並び、役割による差がx方向にしか出ず、横一列のまま平行移動する固定陣形に見えていた
- [x] `target.y` も `home.y` とブレンドするよう修正 `player` — `GameConfig.ai.positioning.receivingHomeBlendY` を追加
- [x] balance-check（20シード）でブレンド比率を0/0.05/0.1/0.15/0.2/0.3/0.5で比較 — 0.2までは平均得点5.10前後を維持するが、**0.3で0.55、0.5で0.00（全試合引き分け）まで急落する非線形な閾値を発見**（過去に何度も見た「事後クランプ・強いブレンド」系の失敗パターンと同種）。閾値からの安全マージンを見て **0.15を採用**（平均得点5.20、勝敗分布も無効時に近い）
- [x] typecheck・全138件のテスト・headless実行で確認（既存テストは無改修のまま全通過）

#### ボール奪取直後の「孤立した単独前進」を緩和（2026-08-08、実試合を見てのユーザー指摘）

「ボールを奪った後に味方が前方にいないのに、前に進もうとするのが不自然」という指摘を受けて `decidePossessionAction` のドリブルフォールバック（パス/シュートの機会がないときの処理）を修正した。

- [x] **原因判明** — パス候補もシュート機会もない場合、味方の状況を一切見ずに常に全力でゴールへドリブルしていた。前方（攻撃方向）に味方が誰もいない孤立した状態でも、単独で敵陣まで駆け上がろうとするため不自然に見えていた
- [x] 前方（`soloDribbleSupportMargin`=2m以上）に味方が誰もいなければ、方向はそのまま（攻撃ゴール）に、速度だけ `soloDribbleSpeedFactor` の分だけ落とすよう修正 `player` — `GameConfig.ai.soloDribbleSpeedFactor`/`soloDribbleSupportMargin` を追加。方向（受け手ポジション算出のような幾何学的な変更）ではなく速度のみを触ることで、過去に何度も踏んだ非線形崩壊のリスクを避けた
- [x] balance-check（20シード）で `soloDribbleSpeedFactor` を0〜1.0で比較 — 全域で平均得点4.75〜5.35に収まり、**過去のポジショニング系の変更と違って非線形な崩壊は一切見られなかった**（無効時5.20）。視覚効果とのバランスを見て**0.5を採用**（平均得点5.10）
- [x] `tests/game/player.test.ts` に「前方に味方がいるときより孤立時の方が遅くドリブルする」テストを追加。typecheck・全139件のテスト・headless実行で確認
...
### マイルストーンK: 受け手ポジショニング再設計（方式A・C・D・E実装済み・アプローチの限界に達し終了）
...
- [x] 目標未達のまま2026-08-06時点で検証を打ち切り。再挑戦するなら `selectPassReceiver` と `computeTargetPosition` を統合する、より大掛かりな再設計が必要（`config.ai.offside.enabled` は `false` のまま）
- [x] ユーザー承認のもと方式E（統一スコアリング）を `feature/receiver-positioning-redesign` ブランチで実装 `player` — 新設した共有スコア関数 `scoreReceivingSpot`（前進度・オフサイドライン超過量・マーク圧力）を `selectPassReceiver`（誰にパスするか）と `computeTargetPosition`（どこへ動くか）の両方が使うようにした。`avoidChance` フィールドは撤去（確率的除外ではなく連続スコアで自然に回避されるため不要になった）
- [x] `offsideOvershootWeight` を2〜200でbalance-check・自然なオフサイド率を計測 — **16以上で自然なオフサイド率が18〜19%まで下がり、初めて目標(20%未満)を達成**。方式Dの51%止まりから大幅改善し、診断（受け手選定とポジショニングの断絶）が正しかったことを裏付けた
- [x] しかし目標達成後も反則有効化時の平均得点は0.10〜0.55点/試合のままで方式Dと同水準の壊滅的偏りが継続。「反則の再開ルール（位置）が原因では」という仮説を実験（再開地点を常に中央に変更、検証後リバート）で検証したが、得点はほぼ変わらず（0.10→0.50）反証された。座標系上、オフサイド地点は反則を受ける側自身の守備エリアにあたるため、再開位置は攻撃側への贈り物になっていないと判明
- [x] 結論として「自然なオフサイド率20%未満なら安全」という前提条件自体が誤りだったと判断。真因はこのAIの攻撃がほぼ縦パス依存で代替手段（ドリブル等）が弱く、20%弱の頻度でも縦パスの完全ターンオーバーに耐えられない構造的な脆さにあると考えられる（詳細: `specification/features_positioning_redesign.md`「方式E 実装結果」）
...
### マイルストーンM: オフサイド反則有効化時の得点崩壊、根本原因の追加調査（2026-08-09）— ⏸ 未解決のまま一旦区切り

マイルストーンK/Lの結論（「攻撃側の代替手段の強化」か「反則発生率を一桁台まで下げる」が必要）を受け、前者の方向で3つの改善を試したが、いずれも崩壊（反則有効化時の平均得点がほぼ0）を止められなかった。検証はすべて一時スクリプト（`scratch_*.ts`、都度削除）で実施。

- [x] `offsideRiskDribbleBoost` を実装 `player` — `selectPassReceiver` が選んだ受け手が実際にオフサイドの場合、`dribbleChance` を加算してリスキーな縦パスを避けやすくする（`avoidanceEnabled=false`の既定ではno-op）。20シードで検証: リスキーなパス試行を35→17→0（boost 0/0.5/1.0）まで確実に減らせたが、**平均得点は0.00〜0.05のままで崩壊は解消せず**
- [x] `keepDribbleEvasionBase`/`keepDribbleEvasionAggroSpread` を実装 `player` — マークされている孤立時ドリブルで、ゴール方向とマーカーから離れる方向を`aggressiveness`に応じてブレンドする「キープドリブル」（打開ではなく味方の動きを待つ狙い）。通常時のbalance-check（100シード）では平均得点9.59→8.50（約11%減、崩壊ではない）と健全に機能。**しかしオフサイド有効時の平均得点は0.07のまま変化なし**（30シード）
- [x] 保持スペル（ボールを持ち続ける一連の区間）ごとの前進距離を計測する手法を確立 — 通常時はパスを挟むスペルの平均前進距離が単独ドリブルの3倍以上（33.20m vs 10.26m）で、パス自体は機能しているとわかった。一方オフサイド有効時はパスを挟むスペルの割合が22%→65%に急増するのに平均前進距離が**マイナス（-0.91m）**になっており、`avoidanceEnabled`時の受け手ポジショニングが前進ではなくむしろ後退する位置を選んでいることを確認
- [x] `scoreReceivingSpot`の`kpp.forwardWeight`/`offsideOvershootWeight`をチューニング — `offsideOvershootWeight`を下げる（安全マージンを緩める）とパス絡みスペルの前進距離をマイナスからプラス（+1.18m）に転換できたが、**それでも平均得点は0.10のままでほぼ改善せず**。受け手位置の質より支配的な崩壊要因が別にあると判断
- [x] 「ピッチが狭すぎるのでは」という仮説を検証 — ピッチを1.5倍・2倍に拡大しても平均得点は改善せず（0.10→0.00→0.00）、むしろ悪化。1人あたりの面積を計算すると本シミュレーション（625㎡/人）は実サッカー11人制（291㎡/人）より広く、**物理的な空間不足が原因ではないと判断**
...
- [x] `resolvePlayerPlayer`/`resolveAllPlayerCollisions` を実装 `collision` — 選手2人の距離が `player.radius * 2` 未満なら重なり量を半分ずつ押し戻す**位置補正のみ**の実装（`vel` には触れない）。速度ベースの反発力にすると、震え対策（moveToward の到達減速、`player.ts` 参照）と同種の押し出し↔押し戻しの振動を再現しかねないと判断し避けた。3人以上のクラスタ状の重なりに対応するため全ペアを10回リラクゼーションする（`COLLISION_RELAXATION_ITERATIONS`）。中心が完全一致する縮退ケースは id の文字列比較で決定的に軸を割り振り、乱数を使わず再現性を保つ
- [x] `Simulator.step` に組み込み — `stepPlayer`（選手移動）の直後・`stepBall`/`resolveBallPossession`（ボール判定）の前に `resolveAllPlayerCollisions` を呼ぶ。既存の距離パラメータ（`tackleDistance=1.0` が押し出し後の最小中心間距離 `1.0m` とちょうど一致するなど）と衝突しないことを確認済み
- [x] テスト追加 `tests/game/collision.test.ts`（単体: 2人の押し出し・境界値・縮退ケース・6人クラスタでの収束）、`tests/simulation/simulator.test.ts`（統合: 固定シードでフルマッチを回し、常に2選手間距離が `radius*2` 以上であることを回帰検証）
